...
K = [Constant(2E-9), Constant(2E-9), Constant(2E-9), Constant(2E-9), Constant(2E-9), Constant(2E-9)]   #[m^2/Pa.s]
#beta = [[0., 0.02, 0., 0.], [0.02, 0., 0.05, 0.], [0., 0.05, 0., 0.1], [0., 0.0, 0.1, 0.]]
beta = [[0., 3E-5, 0., 0., 0., 0.], [3E-5, 0., 3E-5, 0., 0., 0.], [0., 3E-5, 0., 3E-5, 0., 0.], [0., 0., 3E-5, 0., 3E-5, 0.],[0., 0., 0., 3E-5, 0., 3E-5], [0., 0., 0., 0., 3E-5, 0.]] #[1/Pa.s]
#f = [Constant(1200), Constant(0.), -0.1*(p[1]-3)]
f = [Constant(0.), Constant(0.), Constant(0.), Constant(0.), Constant(0.), Constant(0.)]
...
p4_file = XDMFFile("results_"+str(N)+"/p4.xdmf")
p5_file = XDMFFile("results_"+str(N)+"/p5.xdmf")
...
v4_file = XDMFFile("results_"+str(N)+"/v4.xdmf")
v5_file = XDMFFile("results_"+str(N)+"/v5.xdmf")
...
while n<3:
	...
	for i in range(steps):
		...
		v5 = project(-K[4]*grad(p5),W.sub(5).collapse())

		#Z = dot((c - c_n)/dt,s)*dx + dot(dot(v0,grad(c)),s)*dx + dot(epsilon*grad(c),grad(s))*dx - dot(f[0],s)*ds
		#solve (Z==0, c, bcs_react)
		
		# saving solutions
		p0_file.write_checkpoint(pD, 'p0', t+t*n, append = append)
		p1_file.write_checkpoint(p1, 'p1', t+t*n, append = append)
		p2_file.write_checkpoint(p2, 'p2', t+t*n, append = append)
		p3_file.write_checkpoint(p3, 'p3', t+t*n, append = append)
		p4_file.write_checkpoint(p4, 'p4', t+t*n, append = append)
		p5_file.write_checkpoint(p5, 'p5', t+t*n, append = append)

		v0_file.write_checkpoint(v0, 'v0', t+t*n, append = append)
		v1_file.write_checkpoint(v1, 'v1', t+t*n, append = append)
		v2_file.write_checkpoint(v2, 'v2', t+t*n, append = append)
		v3_file.write_checkpoint(v3, 'v3', t+t*n, append = append)
		v4_file.write_checkpoint(v4, 'v4', t+t*n, append = append)
		v5_file.write_checkpoint(v5, 'v5', t+t*n, append = append)
