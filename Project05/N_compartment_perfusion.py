...
# Define parameters: IT MUST BE CHANGED DEPENDING ON HOW MANY COMPARTMENTS WILL BE USED!
K = [Constant(1), Constant(0.5), Constant(0.25), Constant(0.15)]
beta = [[0., 0.5, 0.3, 0.2], [0.5, 0.2, 0.25, 0.2], [0., 0.25, 0.5, 0.2], [0., 0.25, 0.5, 0.2]]
...
p = TrialFunction(P)
q = TestFunction(P)
p_D = Function(P.sub(0).collapse())
R = sum([K[i] * inner(grad(p[i]), grad(q[i])) for i in range(N-1)])*dx + sum([beta[i][i+1]*(p[i]-p[i+1])*q[i]*dx + beta[i+1][i]*(p[i+1]-p[i])*q[i+1]*dx for i in range(N-2)]) - sum([f[i]*q[i] for i in range(N-1)])*dx
...
for i in range(steps):
    ...
    (p1, p2, p3) = p.split(True)    #IT DEPENDS ON HOW MANY COMPARTMENTS  (N-1)
    
    #This is the saving file. It must be created one for each compartment (N)    
